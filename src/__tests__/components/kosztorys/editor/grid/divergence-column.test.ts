...
// „Rozjazd" is the only column whose EXISTENCE is data-driven — every other one is there or hidden by a
// preference. So what is asserted here is the gate, not the arithmetic (measure-discrepancy.test.ts
// owns that): it appears with the first rozjazd, and it is unreachable from a client's document.
...
describe('the „Rozjazd" column', () => {
  it('is absent while nothing diverges', () => {
    expect(ids()).not.toContain('divergence')
    expect(ids({ hasDivergence: false })).not.toContain('divergence')
  })

  // First, ahead of „Sekcja" — a work list you have to scroll to is one you don't read.
  it('leads the rozpiska once a pozycja diverges', () => {
    expect(ids({ hasDivergence: true })[0]).toBe('divergence')
  })
  ...
  it('sits behind the row-actions column when editing is enabled', () => {
    expect(ids({ hasDivergence: true, onRemoveItem: () => {} })).toEqual([
      'actions',
      ...ids({ hasDivergence: true }),
    ])
  })
  ...
  it('never reaches the client preview', () => {
    expect(ids({ hasDivergence: true, previewVisible: true })).not.toContain('divergence')
  })
  ...
  it('is absent on a subcontractor plane', () => {
    expect(ids({ hasDivergence: true, view: 'w_tools' })).not.toContain('divergence')
  })

  it('carries both figures for the cell to render', () => {
    const column = buildV2Columns({ view: 'client', stages: STAGES, hasDivergence: true }).find(
