...
type DescriptionPropsT = {
  ...
  // A note that appears in response to an action (an upload failure) has to announce itself; a note
  // present from first render must not. Only the former passes this.
  role?: 'alert'
}
...
export function Description({
  ...
  return (
    <p
      role={role}
      className={cn('flex items-start gap-1 font-normal', color, SIZE[size].text, className)}
    >
