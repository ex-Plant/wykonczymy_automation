...
| 9   | The client's offer is unreadable — a work name is clipped in the investor preview, and the client has no editor, no handle and no popover to recover it | Medium | Medium     | EX-699: the owner's first request after launch; the grid virtualizes columns, so the measurement that sizes a row can silently read the wrong column |
