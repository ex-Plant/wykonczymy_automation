...
| S-12 | robocizna-from-kosztorys        | see investment robocizna + rabat derived from the kosztorys, not manual transfers       | S-11               | — (owner request)             | implementing | —          |
...
- **Parallel with:** the editor slices (S-02–S-10)
- **Blockers — two open, both the same trap on different figures.** Each gates this slice's **archive** (not its implementation); detail in `context/changes/robocizna-from-kosztorys/open-questions.md`.
...
- **Risk:** this reassigns the source of two figures that feed marża and bilans — the highest-consequence read change in the arc. Guardrail: the explicit per-investment flag + the fallback, so no investment silently changes its numbers. Open: listing-page perf, and the fate of existing `LABOR_COST`/`RABAT` rows on live investments (see `research.md` → "Decided model").
- **Status:** implementing — branch `kosztorys-bridge` (change.md `implementing`, 2026-07-19). **Not archivable while EX-536 / EX-539 are open**: the whole figure of this slice is a mismatch alarm, so getting the netto/brutto axis wrong on either figure makes it fire on healthy investments — the one failure mode that destroys the instrument's value. **Update (EX-536):** the deposit `vatPlane` entry-axis shipped as presentation-only (tryb-mieszany reconciliation) and does **not** yet feed this slice's netto↔netto comparison, so it doesn't unblock archive on its own — the open question is whether the reconciliation should feed the P&L. Note `change.md` still says the kosztorys side is read **gross**; that is stale — superseded by the netto ↔ netto working model above.
