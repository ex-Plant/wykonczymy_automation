...
- [x] fixed · `feature-first` · `src/components/forms/form-fields/plane-amount-field.tsx:1`, `expense-category-field.tsx:1` · oba mają dokładnie jednego konsumenta-formularz i po EX-733 importują typ API TEGO formularza — czyli siedzą we wspólnym `form-fields/`, będąc de facto polami jednego formularza. Gałąź ich nie przeniosła (status `M`), ale to ona uczyniła rozjazd widocznym. **Decyzja w kroku 2.** — **fixed:** przeniesione do `deposit-form/` i `edit-transfer-form/`, zdjęte z `form-fields/index.ts`, import prosto w miejscu renderu

### module-cohesion-audit — 4 realne, 3 marginalne

- [x] fixed · `cohesion` · `src/lib/invoices/upload-file-client.ts:42,58` · `positionalFiles` i `filesByRowId` to czyste rzutowania id↔pozycja nad `lineItems` — nic nie wysyłają, a ich jedyni konsumenci to komponenty formularza. Plik zmienia się i przy transporcie do Bloba, i przy kontrakcie wiersz↔plik. **Do wydzielenia w kroku 2** (`row-file-positions.ts` + spek za nim). — **fixed:** `positionalFiles`/`filesByRowId` wyniesione do `lib/invoices/row-file-positions.ts`; spek przeniesiony na lustro ścieżki (`__tests__/lib/invoices/row-file-positions.test.ts`)
...
- [x] fixed · `cohesion` · `src/components/forms/form-fields/plane-amount-field.tsx:23` · `netSuggestion()` to arytmetyka VAT bez Reacta, eksportowana z `.tsx` wyłącznie po to, żeby dosięgnął jej spek. **Do wydzielenia w kroku 2.** — **fixed:** `netSuggestion()` wyniesione do `lib/utils/net-suggestion.ts`, spek za nim
...
- [x] fixed · `reuse` · `src/components/forms/deposit-form/deposit-form-api.ts:19` · wyprowadza typ API formularza ręcznym `ReturnType<typeof useManagedForm<…>>['form']`, podczas gdy rodzeństwo dodane tym samym diffem (`edit-transfer-form-api.ts:8`) i plik zmieniony tym samym diffem (`bulk-expense-form.ts:58`) idą przez `FormApiOfT` — prymityw, który **ta gałąź wprowadziła** (`forms/hooks/form-api-of.ts:16`). Oba komentarze podają identyczne uzasadnienie. `useManagedForm` zwraca formularz `useAppForm` bez zmian, więc rozjazd nie ma powodu. **Do ujednolicenia w kroku 2.** — **fixed:** `DepositFormApiT = FormApiOfT<DepositFormValuesT>`
...
- [x] fixed · `comment-noise` · `src/components/kosztorys/editor/grid/kosztorys-v2-columns.tsx:99` · **komentarz sierota** — uzasadnienie „«Cena j.m.» to pieniądze, więc rollback nazywa je w złotówkach" stoi na poziomie modułu, oderwane od swojego kodu o ~185 linii; w tym miejscu czyta się jako komentarz do `PLANE_UNCONFIRMED_CELL`. Treść wartościowa — **przenieść** do wywołania koło :285, nie kasować. — **fixed:** komentarz przeniesiony do wywołania `decimalColumn` dla „Cena j.m.”
- [x] fixed · `comment-noise` · `src/__tests__/lib/kosztorys/cell-edit.test.ts:150` · jedyny komentarz po polsku dodany tym diffem (tytuły `it(...)` po polsku to konwencja i zostają). Reguła repo: komentarze zawsze po angielsku, a ta treść stoi już po angielsku w `discount-edit.ts:17`. — **fixed:** przetłumaczony na angielski
- [x] fixed · `comment-noise` · 2 do skasowania + 6 do przycięcia · `backfill-heic-media.ts:266` (banner „Phase B" powtarza `console.log` linijkę niżej), `submit-with-invoice-pages.ts:46` (`/** One invoice's pages, flat. */` = sygnatura), oraz przycięcia w `use-undo-redo.ts:32`, `use-invoice-removal.ts:75`, `use-cell-draft.ts:31` i `:112`, `undo-coalesce.ts:29`, `expense-form.tsx:226` („keep them green before touching this" to instrukcja procesowa, nie uzasadnienie). **Do zastosowania w kroku 2.** — **fixed:** skasowane oba bannery, przycięte `use-undo-redo.ts`, `use-cell-draft.ts`, `undo-coalesce.ts`
...
- [x] fixed · `test-coverage` · `code-review` · `src/lib/utils/decimal-text.ts:11` + `parse-decimal-input.ts:23` · dwa nowe helpery, których cały kontrakt polega na tym, że są swoimi odwrotnościami — i nic nie sprawdza obiegu. `parse-decimal-input.ts:9-10` mówi to wprost. Spek pokrywa tylko wcześniejszy `parseDecimalInput`; **ani `decimalText`, ani `parseCellDecimal` nie ma nigdzie żadnej asercji**. A to dosłownie kopiuj→wklej w edytorze kosztorysu (`decimalText` jest `copyValue` kolumn, `parseCellDecimal` czyta wklejkę z powrotem). Obie ścieżki awarii dziś trzymają (notacja wykładnicza `String()` parsuje się z powrotem; artefakty float są uniknięte, bo jedyna liczona wartość idzie przez `roundToCents`) — obie są o jeden refaktor od nietrzymania i nic by nie padło. — **fixed:** `__tests__/lib/utils/parse-decimal-input.test.ts` — round trip `parseCellDecimal(decimalText(x))` na tabelce wartości, wklejka z NBSP, forma ścisła, null/undefined
      test: TDD · unit — `parseCellDecimal(decimalText(x))` zwraca `{kind:'value', value:x}` dla tabelki z wklejką rozdzieloną NBSP, wartością ujemną i ceną po `roundToCents`
